...
- [x] `ContactSubmissionsManager.jsx` — removed fake Django admin chrome (#dj-header bar, WELCOME/logout links, breadcrumbs, "Select Contact Submission to change" page title); now renders only the actual submissions content (object-tools, search toolbar, bulk-action bar, result table, right-side filter panel, toast); removed unused header/breadcrumb CSS
